<template>
  <div class="lg:ml-[250px] lg:mr-[100px] pt-[50px] flex flex-col gap-5 text-black">
    <div class="flex flex-row items-center gap-5">
      <div class="size-[50px] flex justify-center items-center cursor-pointer text-[#508aff] hover:text-[#a0c0ff] transition duration-200">
        <Icon name="lets-icons:back" size="40" />
      </div>
      <h2 class="font-semibold">
        Тақырып аты: <span class="text-blue-600 text-xl">{{ data }}</span>
      </h2>
    </div>

    <div class="lg:w-full lg:min-h-[60vh] bg-[#A6E3E9] rounded-lg border-2 border-sky-400 flex justify-center items-center">
      <div v-if="taskOptions.loading" class="text-center py-4 text-green-700">
        ЩААА...
      </div>
      <div v-else-if="countTaskOptions.valueOf === 0" class="text-black text-center mt-[5vh] italic font-semibold text-2xl">
        ...
        <div v-if="photos.length > 0" class="flex flex-col gap-3">
          ...
          <div class="flex flex-wrap gap-4 items-center">
            <div v-for="(photo, index) in photos" :key="index" class="relative w-30 h-30 border rounded overflow-hidden">
...
onMounted(async () => {
  ...
  countTaskOptions.value = taskOptions.task_options.length;
